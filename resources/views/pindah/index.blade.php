...
@section('content')
    <div class="row">
        <div class="col-lg-12">
            <x-card>
                <x-slot name="header">
                    <button onclick="addForm(`{{ route('data_pindah.store') }}`)" class="btn btn-success"><i
...
<script>
    ...
    table = $('.table').DataTable({
        ...
        responsive: true,
        ajax: {
            url: '{{ route('data_pindah.data')}}',
        },
        columns: [
            {data: 'DT_RowIndex', searchable: false},
            {data: 'nik', sortable: false},
            {data: 'nama_lengkap', sortable: false},
            {data: 'tanggal_pindah', searchable: false, sortable: false},
            {data: 'alamat_tujuan_pindah', searchable: false, sortable: false},
            {data: 'keterangan_pindah', searchable: false, sortable: false},
            {data: 'action', searchable: false, sortable: false}
        ]
    });

    function addForm(url, title = 'Tambah Data Pindah') {
        $(modal).modal('show');
    ...
    function editForm(url, title = 'Edit Data Pindah') {
        $.get(url)
            .done(response => {
                $(modal).modal('show');
                $(`${modal} .modal-title`).text(title);
    ...
    function resetForm(selector) {
        ...
        $('.form-control, .custom-select, [type=file], .select2, .note-editor').removeClass('is-invalid');
